...
def createPetriStyleArray(image, skip=1):
    h, w = image.shape[:2]
    black_dots = []
    # Process each block
...
def main():
    # this assumes the image is already at the right size and has a contour or a final shape so it just converts it into an array of coordinates suitable for a Petri dish operation
    path="C:/a/diy/pythonProjects/labRobot/src/image/" #change here
    #image_path =path +"tinyLeafcontour.png" # change here
    image_path =path +"tinyLeaf.png" # change here
    skipping=2 # change here
    image = cv2.imread(image_path)
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)    	
    (thresh, blackAndWhiteImage) = cv2.threshold(gray, 254, 255, cv2.THRESH_BINARY)
    imageInfo(blackAndWhiteImage,"image_path")
    cv2.imshow("image", blackAndWhiteImage)
    cv2.waitKey()
    centeredDotArray=createPetriStyleArray(blackAndWhiteImage, skip=skipping)
    ...
    height,width=blackAndWhiteImage.shape[:2]
